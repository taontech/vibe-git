...
async function testBackgroundWebLifecycle(tempDir, fakeServerPath) {
  ...
  try {
    ...
    assert(startOutput.indexOf('Opening http://127.0.0.1:' + webPort) >= 0, startOutput);
    ...
    assert(runningOutput.indexOf('Opening http://127.0.0.1:' + webPort) >= 0, runningOutput);
...
function fakeServerSource() {
  return [
    ...
    "    return;",
    "  }",
    "  if (req.url === '/agents' || req.url.indexOf('/status') === 0) {",
    "    res.writeHead(200, { 'Content-Type': 'application/json' });",
    ...
    "          plan: 'Plus',",
    "          windows: [",
    "            { label: '5h', used_percent: 35.0 },",
    "            { label: '7d', used_percent: 62.5 }",
    "          ]",
    "        }",
    "      }",
    "    }));",
...
async function assertPausedAgentMonitor(port, testDir) {
  ...
  assert.ok(body.usage, response.body);
  assert.strictEqual(body.usage.codex.status, 'ok', response.body);
}
